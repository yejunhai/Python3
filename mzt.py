...
def out_log(text):
    with open(f'{sys.argv[0].split(".")[0]}.log', mode='a') as file:
...
class Check():
    ...
    def disk(self):
        if self.switch[3] == 1:
            ...
            # return sum(disk_use_sum) 磁盘使用率:{disk_all_use}%
...
for info_row in info_table:
    ...
    db.commit()
    out_log(
        f"{cur_time} {ip} CPU使用率:{cpu_use}% 内存使用率:{mem_use}%  网卡速率:{speed_use}kB/s 端口{port} ESTABLISHED并发连接数{coonect_use[0]} 全状态连接数:{coonect_use[1]}")
db.commit()
db.close()
print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
